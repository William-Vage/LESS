...
args.table_file = os.path.join(extract_path, 'my_calibration_table.json') #"../../data/correct/my_calibration_table.json"
args.dataset_flag = 1 
...
def query(dataset):
        ...
        start_nodes = []
        if dataset == 'leonard':
            start_nodes = [44238] #[edges[2]]
        elif dataset == 'darpa_tc':
            start_nodes = [14283]
        
        global o1
        o1=edges[3] #边的parent id
        global o2
        o2=edges[2] #边的children id
        ...
        for start in start_nodes:
            ...
            for j in range(int(total_counter/batch_size)+addd): #对于每一个batch
                if addd==1 and j==int(total_counter/batch_size): #最后小batch的情况
                    ...
                    final_sentences=predict_lstm(queries,counters_,timesteps, alphabet_size,batch_size*j,ver_len,-1) #完成模型预测详细信息恢复[2, 3, 4, 5, 3, 6, 7, 8, 10, 11, 12, 15, 18, 18, 12, 0, 11, 0, 11, 0, 10, 16, 12, 18, 0, 15, 18, 0, 18, 13, 15, 15, 0, 9, 0, 9, 0, 10, 11, 12, 15, 18, 18, 12, 0, 1]
                ...
                for i in range(len(final_sentences)):
                    ret = translate(final_sentences[i],flag=0)
                    if len(ret) != 0:
        ...
        # 指定保存的文件名
        output_file = "data/query_result/final_results.json"
...
def leonard_query_run(dataset='leonard'):
    begin_time=time()
    out_zip_file = ''
    if dataset == 'leonard':
        out_zip_file = os.path.join(config.project_root, 'data/compress/leonard_output.zip')
    elif dataset == 'darpa_tc':
...
if __name__ == "__main__":    
    leonard_query_run(dataset='leonard')
